...
def odom_callback(msg):
    global car_x, car_y, car_rad, car_deg
...
def path_callback(msg):
    ...
    for pose_stamped in msg.poses:
        p_ = Point()
        p_.x = pose_stamped.pose.position.x
        p_.y = pose_stamped.pose.position.y
...
def run():
    ...
    rospy.Subscriber('/obs_global_path',Path , path_callback)
    ...
    while not rospy.is_shutdown():
        ...
        for steering_deg in range(-30,30,9):
        ...
        for path_idx, path_ in enumerate(all_paths):
            ...
            for path_point in path_:
                ...
                for op in obscell_copy:
                    # occ_map은 velodyne 기준이고 path는 base link 여서 이런 문제가 발생하였음. 
        ...
        if len(safe_path_idx) == max_safe_len: 
            ...
            for idx in safe_path_idx:
                ...
                if len(gpath) > 0:
                    for gp in gpath:
                        dist = distance(plp_x, plp_y, gp.x, gp.y)
                        # print('dist' ,dist)
                        if dist < min_dist:
                            min_dist = dist
                else:
            ...
            # nearest_pathpoint = Point()
            # # 가장 가까운 경로상 포인트 구하기
            # min_d = 100000
            # min_idx = -1
            # for gpidx, gp in enumerate(gpath):
            #     d = distance(gp.x, gp.y, car_x, car_y)
            #     if d < min_d:
            #         min_d = d
            #         min_idx = gpidx
